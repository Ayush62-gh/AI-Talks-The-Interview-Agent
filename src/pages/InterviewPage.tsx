...
export default function InterviewPage() {
  ...
  useEffect(() => {
    if (!loading && messages.length === 0 && !sessionId) {
      navigate('/setup');
    }
  }, [loading, messages.length, sessionId, navigate]);
  ...
  return (
    <main className="lg:h-screen lg:overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.1),transparent_35%),radial-gradient(circle_at_bottom_right,_rgba(168,85,247,0.12),transparent_25%)] p-3 text-slate-900 transition-colors duration-300 sm:p-4 dark:text-slate-100">
      <Toast
        message={error ?? ''}
      ...
      <div className="grid lg:h-full w-full max-w-none px-1 sm:px-3 lg:px-4 gap-4 lg:grid-cols-[320px_1fr] xl:grid-cols-[350px_1fr]">
        <section className="flex flex-col justify-between lg:h-full lg:overflow-y-auto rounded-[2rem] border border-slate-200/70 bg-white/75 p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/70 dark:shadow-glass">
        ...
        <section className="flex lg:h-full min-h-[500px] flex-col rounded-[2rem] border border-slate-200/70 bg-white/75 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/70 dark:shadow-glass">
          <div className="border-b border-slate-200/70 px-6 py-4 text-sm text-slate-600 dark:border-white/10 dark:text-slate-300">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-slate-700 dark:border-white/10 dark:bg-slate-900/70 dark:text-slate-300">
          ...
          <form onSubmit={handleSubmit} className="sticky bottom-0 border-t border-slate-200/70 bg-white/90 px-6 py-4 backdrop-blur-md dark:border-white/10 dark:bg-slate-950/90">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
              <div className="relative flex-1">
                <textarea
                  ...
                  className="min-h-[90px] w-full resize-y rounded-[1.35rem] border border-slate-200/70 bg-slate-50/90 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-400/20 disabled:cursor-not-allowed disabled:opacity-70 dark:border-white/10 dark:bg-slate-900/90 dark:text-slate-100"
                />
                <div className="pointer-events-none absolute bottom-2 right-4 text-xs text-slate-500 dark:text-slate-400">
